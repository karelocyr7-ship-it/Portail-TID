...
export default function EmailAdministrationPage() {
  ...
  const test = async () => {
    if (
      !window.confirm(
        "Envoyer un message de test à la destination configurée ?",
      )
    )
      return;
    setBusy(true);
    ...
    setMessage(
      response.ok
        ? "Test SMTP envoyé."
        : (data.error ?? "Le test SMTP a échoué."),
    );
    setBusy(false);
    await refresh();
  };
  ...
  return (
    <div className="page-container admin-page">
      <section className="admin-hero">
        <div>
          <p className="eyebrow light">Administration</p>
          <h1>Paramètres e-mails.</h1>
          <p>État du transport SMTP transactionnel du portail.</p>
        </div>
      </section>
      <section className="admin-stat-grid" aria-label="État SMTP">
        <div className="admin-stat-card">
          ...
          <span>
            <strong>{status?.enabled ? "Activé" : "Désactivé"}</strong>
            <small>SMTP</small>
          </span>
        </div>
        <div className="admin-stat-card">
        ...
        <div className="admin-stat-card">
          ...
          <span>
            ...
            <small>En échec</small>
          </span>
        </div>
        <div className="admin-stat-card">
          <span className="admin-stat-icon">↗</span>
          <span>
            <strong>{status?.sent24h ?? "—"}</strong>
            <small>Envoyés (24 h)</small>
          </span>
        </div>
      </section>
      <section className="panel-card">
        ...
        >
          {busy ? "Test en cours…" : "Tester la configuration SMTP"}
        </button>
