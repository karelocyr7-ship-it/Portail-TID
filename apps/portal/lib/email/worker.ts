...
function errorInfo(error: unknown): {
  ...
  if (error instanceof EmailPermanentError)
    return { code: error.code, message: error.message, temporary: false };
  if (error instanceof EmailTemporaryError)
    return { code: error.code, message: error.message, temporary: true };
  return {
    code: "EMAIL_WORKER_ERROR",
    message: "Unexpected email worker failure",
    temporary: true,
  };
}
...
async function processOne(workerId: string): Promise<boolean> {
  ...
  try {
    const result = await sendTransactionalEmail({
      ...
      variables: (email.templatePayload ?? {}) as Record<
        string,
    ...
    const delay =
      retryDelaysMs[Math.min(email.attempts, retryDelaysMs.length - 1)] ??
      retryDelaysMs.at(-1)!;
    ...
    console.error(
      JSON.stringify({
        event: terminal ? "email.failed" : "email.retry",
        id: email.id,
        template: email.templateName,
        attempt: email.attempts,
        code: info.code,
