...
public class ConfigureModule extends AbstractModule {
    ...
    private final String oidcEnabledParameter = "oidc.enabled";
    private final String oidcIssuerParameter = "oidc.issuer";
    private final String oidcClientIdParameter = "oidc.client.id";
    private final String oidcClientSecretParameter = "oidc.client.secret";
    private final String oidcRedirectUriParameter = "oidc.redirect.uri";
    ...
    protected void configure() {
        ...
        opt = this.context.getInitParameter(oidcEnabledParameter);
        this.bindConstant().annotatedWith(Names.named(oidcEnabledParameter)).to(
                opt != null && (opt.equals("1") || opt.equalsIgnoreCase("true")));
        opt = this.context.getInitParameter(oidcIssuerParameter);
        this.bindConstant().annotatedWith(Names.named(oidcIssuerParameter)).to(opt != null ? opt : "");
        opt = this.context.getInitParameter(oidcClientIdParameter);
        this.bindConstant().annotatedWith(Names.named(oidcClientIdParameter)).to(opt != null ? opt : "");
        opt = this.context.getInitParameter(oidcClientSecretParameter);
        this.bindConstant().annotatedWith(Names.named(oidcClientSecretParameter)).to(opt != null ? opt : "");
        opt = this.context.getInitParameter(oidcRedirectUriParameter);
        this.bindConstant().annotatedWith(Names.named(oidcRedirectUriParameter)).to(opt != null ? opt : "");
