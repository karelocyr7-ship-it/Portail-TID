angular
  ...
  .controller(
    ...
    function (
      ...
      if (!$scope.localLogin) {
        authService.oidcConfig(function (response) {
          if (response && response.enabled) {
            $scope.oidcEnabled = true;
            $scope.startOidc();
          }
        });
      }
      ...
      rebranding.query(function (value) {
        ...
        if (
          $scope.rebranding.signupLink == "https://h-mdm.com/contact-us/" &&
          getBrowserLanguage() == "ru_RU"
        ) {
          $scope.rebranding.signupLink = "https://h-mdm.com/ru/kontakty/";
        }
        $scope.ieBrowserNotice2 = localization
          .localize("ie.browser.notice.2")
          .replace("${appName}", $scope.rebranding.appName);
      });
      ...
      var loginHandler = function (response) {
        if (response.status === "OK") {
          if (response.data.passwordReset) {
            $state.transitionTo("passwordReset", {
